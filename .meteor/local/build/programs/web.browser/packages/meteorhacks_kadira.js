...
ErrorModel.prototype.sendError = function(errorDef, err, force) {                                   // 22
  ...
  function sendError() {                                                                            // 47
    if(!self.errorsSent[errorDef.name]) {                                                           // 48
      ...
      errorDef.count = 1;                                                                           // 53
      Kadira.send({errors: [errorDef]}, '/errors');                                                 // 54
                                                                                                    // 55
      self.errorsSent[errorDef.name] = _.clone(errorDef);                                           // 56
      self.errorsSent[errorDef.name].count = 0;                                                     // 57
      self.errorsSentCount++;                                                                       // 58
    } else {                                                                                        // 59
      self.increamentErrorCount(errorDef.name);                                                     // 60
    }                                                                                               // 61
  }                                                                                                 // 62
};                                                                                                  // 63
                                                                                                    // 64
ErrorModel.prototype._flushErrors = function() {                                                    // 65
  var self = this;                                                                                  // 66
  var errors = _.values(self.errorsSent);                                                           // 67
  errors = _.filter(errors, function(error) {                                                       // 68
    return error.count > 0;                                                                         // 69
  });                                                                                               // 70
                                                                                                    // 71
  if(errors.length > 0) {                                                                           // 72
    Kadira.send({errors: errors}, '/errors');                                                       // 73
  }                                                                                                 // 74
  self.errorsSent = {};                                                                             // 75
};                                                                                                  // 76
                                                                                                    // 77
ErrorModel.prototype.isErrorExists = function(name) {                                               // 78
  return !!this.errorsSent[name];                                                                   // 79
};                                                                                                  // 80
                                                                                                    // 81
ErrorModel.prototype.increamentErrorCount = function(name) {                                        // 82
  var error = this.errorsSent[name];                                                                // 83
  if(error) {                                                                                       // 84
    error.count++;                                                                                  // 85
  }                                                                                                 // 86
};                                                                                                  // 87
                                                                                                    // 88
ErrorModel.prototype.canSendErrors = function() {                                                   // 89
  return this.errorsSentCount < this.options.maxErrorsPerInterval;                                  // 90
};                                                                                                  // 91
                                                                                                    // 92
ErrorModel.prototype.close = function() {                                                           // 93
  clearTimeout(this.intervalTimeoutHandler);                                                        // 94
};                                                                                                  // 95
                                                                                                    // 96
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                  //
// packages/meteorhacks:kadira/lib/profile/client.js                                                //
//                                                                                                  //
//////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                    //
Kadira.profileCpu = function(arg1, arg2, type) {                                                    // 1
  console.log("Kadira: profiling has been started. check server logs");                             // 2
  Meteor.call('kadira.profileCpu', arg1, arg2, type, function(err, res) {                           // 3
    if(err) {                                                                                       // 4
      console.error("Kadira: profiling CPU attempt failed: " + err.message);                        // 5
    } else {                                                                                        // 6
      console.log("Kadira: " + res);                                                                // 7
    }                                                                                               // 8
  });                                                                                               // 9
};                                                                                                  // 10
//////////////////////////////////////////////////////////////////////////////////////////////////////
