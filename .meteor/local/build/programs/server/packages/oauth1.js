...
OAuth._requestHandlers['1'] = function (service, query, res) {                                         // 4
  ...
  if (query.requestTokenAndRedirect) {                                                                 // 15
    ...
    if (query.oauth_token && query.oauth_token === requestTokenInfo.requestToken) {                    // 66
      ...
      var oauthResult = service.handleOauthRequest(oauthBinding);                                      // 75
                                                                                                       // 76
      var credentialToken = OAuth._credentialTokenFromQuery(query);                                    // 77
      credentialSecret = Random.secret();                                                              // 78
                                                                                                       // 79
      // Store the login result so it can be retrieved in another                                      // 80
      // browser tab by the result handler                                                             // 81
      OAuth._storePendingCredential(credentialToken, {                                                 // 82
        serviceName: service.serviceName,                                                              // 83
        serviceData: oauthResult.serviceData,                                                          // 84
        options: oauthResult.options                                                                   // 85
      }, credentialSecret);                                                                            // 86
    }                                                                                                  // 87
                                                                                                       // 88
    // Either close the window, redirect, or render nothing                                            // 89
    // if all else fails                                                                               // 90
    OAuth._renderOauthResults(res, query, credentialSecret);                                           // 91
  }                                                                                                    // 92
};                                                                                                     // 93
                                                                                                       // 94
/////////////////////////////////////////////////////////////////////////////////////////////////////////
